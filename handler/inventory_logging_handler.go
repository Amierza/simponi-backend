package handler

import (
	"fmt"
	"net/http"

	"github.com/Amierza/simponi-backend/dto"
	"github.com/Amierza/simponi-backend/response"
	"github.com/Amierza/simponi-backend/service"
	"github.com/gin-gonic/gin"
	"go.uber.org/zap"
)

type (
	IInventoryLoggingHandler interface {
		CreateInventoryLog(ctx *gin.Context)
		GetInventoryLogs(ctx *gin.Context)
	}

	inventoryLoggingHandler struct {
		inventoryLogService service.IInventoryLogService
		logger              *zap.Logger
	}
)

func NewInventoryLoggingHandler(inventoryLogService service.IInventoryLogService, logger *zap.Logger) *inventoryLoggingHandler {
	return &inventoryLoggingHandler{
		inventoryLogService: inventoryLogService,
		logger:              logger,
	}
}

func (ilh *inventoryLoggingHandler) CreateInventoryLog(ctx *gin.Context) {
	var req dto.InventoryLogRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		status := mapErrorStatus(err)
		res := response.BuildResponseFailed(fmt.Sprintf("%s", dto.FAILED_CREATE_INVENTORY_LOG), err.Error())
		ctx.AbortWithStatusJSON(status, res)
		return
	}

	result, err := ilh.inventoryLogService.CreateInventoryLog(ctx, req)
	if err != nil {
		status := mapErrorStatus(err)
		res := response.BuildResponseFailed(fmt.Sprintf("%s", dto.FAILED_CREATE_INVENTORY_LOG), err.Error())
		ctx.AbortWithStatusJSON(status, res)
		return
	}

	res := response.BuildResponseSuccess(fmt.Sprintf("%s", dto.SUCCESS_CREATE_INVENTORY_LOG), result)
	ctx.JSON(http.StatusCreated, res)
}

func (ilh *inventoryLoggingHandler) GetInventoryLogs(ctx *gin.Context) {
	productID := ctx.Query("product_id")
	var paginationReq response.PaginationRequest
	if err := ctx.ShouldBindQuery(&paginationReq); err != nil {
		status := mapErrorStatus(err)
		res := response.BuildResponseFailed(fmt.Sprintf("%s", dto.FAILED_GET_INVENTORY_LOGS), err.Error())
		ctx.AbortWithStatusJSON(status, res)
		return
	}

	if paginationReq.Page <= 0 {
		paginationReq.Page = 1
	}
	if paginationReq.PerPage <= 0 {
		paginationReq.PerPage = 10
	}

	result, err := ilh.inventoryLogService.GetInventoryLogs(ctx, productID, paginationReq)
	if err != nil {
		status := mapErrorStatus(err)
		res := response.BuildResponseFailed(fmt.Sprintf("%s", dto.FAILED_GET_INVENTORY_LOGS), err.Error())
		ctx.AbortWithStatusJSON(status, res)
		return
	}

	res := response.Response{
		Status:   true,
		Messsage: fmt.Sprintf("%s", dto.SUCCESS_GET_INVENTORY_LOGS),
		Data:     result.Data,
		Meta:     result.PaginationResponse,
	}
	ctx.JSON(http.StatusOK, res)
}
